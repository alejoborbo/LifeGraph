...
@cli.command()
def setup():
    """First-time setup: sync all sources, extract topics, build graph, serve."""
    _run_full_sync()

    click.echo("\nStarting server at http://localhost:8042\n")
    try:
        import webbrowser
...
def _run_full_sync():
    """Full pipeline: sync all sources → extract topics → build graph."""
    ...
    if not ANTHROPIC_API_KEY:
        click.echo("Error: ANTHROPIC_API_KEY not set. Add it to .env")
    ...
    if sources_synced == 0:
        click.echo("\nNo sources configured. Edit .env (see .env.example).")
        raise SystemExit(1)

    # ── Step 2: Extract topics via Claude API ──────────────
    click.echo("\n── Extracting topics ──")
    docs = get_documents_without_topics()
    if docs:
        click.echo(f"  {len(docs)} document(s) to process...")
    ...
    total_docs = sum(count_documents_by_source().values())
    topic_count = len(get_all_topics_with_counts())
    click.echo(f"  {total_docs} docs → {topic_count} topics → {len(g['nodes'])} clusters → {len(g['edges'])} connections")
    click.echo("\nDone! Run 'lifegraph serve' to view your graph.")


@cli.group(invoke_without_command=True)
@click.pass_context
def sync(ctx):
    """Sync all sources, extract topics, build graph. Or: sync <source> for one source."""
    if ctx.invoked_subcommand is not None:
        return  # A subcommand was given (e.g. sync confluence), let it run
